...
*/
/*-------------------------------------------------------------------------*/
__global__ void Kernel_Find_Adjacency_Matrix_UpTri(
	const int * inimg,
	int * outimg,
	const  gSLICr::Vector2i img_size,
	const  gSLICr::Vector2i adj_size)
{
	...
	if (x < img_size.x - BORDER &&
		...
		find_adjacency_matrix_base_UpTri(inimg, outimg, img_size, adj_size, x, y);
...
*/
/*-------------------------------------------------------------------------*/
void gSLICr::engines::seg_engine_GPU_cluster::Find_Adjacency_Matrix_E_UpTri()
{
	...
	Kernel_Find_Adjacency_Matrix_UpTri << <gridSize, blockSize >> >(inimg_ptr, outimg_ptr, img_size, adj_size);
}
/*-------------------------------------------------------------------------*/
/**
*
...
*/
/*-----------------------------------------*/
void gSLICr::engines::getSameClass(std::vector<int>& sameClass ,const int _ci,const float * _mat, const int _wh)
{
...
void gSLICr::engines::seg_engine_GPU_cluster::Cvt_Labels_to_ClusterLabels_CPU()
{
	...
	const int SZ=WIDTH*HEIGHT;
		...
		for (int pi = 0; pi < SZ; pi++){
			const int LABEL_OLD = labels_idx_ptr[pi];
			const int LABEL_NEW = map_ptr[LABEL_OLD];
			cluster_idx_ptr[pi] = LABEL_NEW;
		}
...
void gSLICr::engines::seg_engine_GPU_cluster::Perform_Cluster()
{
	...
	this->Find_Adjacency_Matrix_E_UpTri();
